...
class smallNORB(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        target = self.labels[index % len(self.infos)] if self.mode == "all" else self.labels[index]
        if self.target_transform is not None:
            target = self.target_transform(target)

        info = self.infos[index % len(self.infos)] if self.mode == "all" else self.infos[index]
